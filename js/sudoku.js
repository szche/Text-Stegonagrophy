...
const createSudoku = (coverTextMatrix, secretTextAscii) => {
  ...
  coverTextLoop: coverTextMatrix.forEach((matrix) => {
    const sudoku = Array.from(Array(9), () => new Array(9));
    sudokuArray.push(sudoku);
    asciiTextLoop: for (let k = 0; k < secretTextAscii.length; k++) {
      firstMatrixLoop: for (let i = 0; i < matrix.length; i++) {
        if (!matrix[i]) new Error("Cover text is too short!");
        secondMatrixLoop: for (let j = 0; j < matrix[i].length; j++) {
          if (!matrix[i][j]) continue firstMatrixLoop;
          letterLoop: for (let z = 0; z < matrix[i][j].length; z++) {
            if (secretTextAscii[k] == matrix[i][j][z]) {
